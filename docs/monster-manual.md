...
!!! quote "👂 Ekkokin"
    ...
    === "Description"
        ...
        *"Homophonic idiolectician. There leased favourite flavour? Orange."*
        ...
        Ekkokin is a cunning glitchling that swaps words for their curated homophones—words that sound identical but have different spellings and meanings. This creates text that reads correctly when spoken aloud but is subtly wrong on the page.
...
!!! quote "📝 Pedant"
    ...
    === "Description"
        ...
        *"Resurrects archaic ligatures and diacritics."*
        ...
        - **Coeurite**: Restores archaic ligatures (œ, æ) to modern words
        - **Whom Stone**: Forces `who` → `whom` for object-pronoun precision
        - **Fewerite**: Swaps `less` for `fewer` with countable quantities
        - **Curlite**: Polishes straight quotes into typographic pairs
        - **Subjunctite**: Corrects subjunctive phrases (`if I was` → `if I were`)
        - **Oxfordium**: Enforces serial commas in simple lists
        - **Metricite**: Converts mile distances to rounded kilometres
        - **Orthogonite**: Unlocks the legendary all-caps Correctopus form
