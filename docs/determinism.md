...
- **Attack helper** – pass `seed=` to `Attack(...)` when providing a sequence; Attack will build a seeded `Gaggle` using `DEFAULT_ATTACK_SEED` when none is given. Supplying an existing `Gaggle` or `Glitchling` resets its RNG when a seed is provided.
