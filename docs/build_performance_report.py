...
from benchmarks.pipeline_benchmark import (  # type: ignore  # pylint: disable=wrong-import-position
    DEFAULT_ITERATIONS,
    BenchmarkResult,
...
def build_report_content(
    *,
    ...
    results = collect_benchmark_results(texts, iterations)
    generated_at = dt.datetime.now(dt.timezone.utc).isoformat()
    rust_available = any(result.rust is not None for result in results)

    lines: list[str] = [
        "# Glitchlings Performance Benchmarks\n\n",
        "This page is generated by `docs/build_performance_report.py` and refreshed as part of the build pipeline. "
        "It compares the pure-Python orchestrator with the optional Rust acceleration layer exposed via "
    ...
    for result in results:
        ...
        lines.append(
            f"| {result.label} | {result.char_count} | {python_cell} | {rust_cell} | {speedup_cell} |\n"
        )
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=(
            "Destination Markdown file. Defaults to docs/performance-comparison.md inside the repository."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    content = build_report_content(iterations=args.iterations)
