...
- **Metrics TUI layout refresh**: rebuilt the Textual interface so the controls live in a fixed-width column and the output/diff/metrics panes share the remaining width via split panes. Token diff/debug panels are now toggleable overlays instead of separate tabs.
- **Responsive helpers**: CSS breakpoints automatically collapse the layout under narrow terminals, mirroring the `set_narrow_mode` logic already in the `MetricsView`.
- **Documentation**: captured a new screenshot (`docs/images/metrics-tui-layout.svg`) and updated the metrics README to describe the new split-pane workflow.
- **Testing**: introduced `tests/test_metrics_tui_layout.py`, a Textual smoke test that mounts the UI at wide and narrow widths and exercises the token diff toggle.
